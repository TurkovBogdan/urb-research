...
@pytest.mark.parametrize(
    ("code", "path"),
    [
        ("RESEARCH@abc0000001", "/research/researches/RESEARCH@abc0000001"),
        ("GROUP@abc0000002", "/research/researches/GROUP@abc0000002"),
        ("AREA@abc0000003", "/research/areas/AREA@abc0000003"),
        ("NOTE@abc0000004", "/research/notes/NOTE@abc0000004"),
        ("QUERY@abc0000005", "/research/queries/QUERY@abc0000005"),
        ("SOURCE@abc0000006", "/research/sources/SOURCE@abc0000006"),
        # Коды web_search длиннее и такими остаются — их агент не видит (см. research.codes).
...
async def test_interface_open_reports_a_host_with_no_browser(call, monkeypatch):
    ...
    with pytest.raises(ToolError, match="No browser to open"):
        await call("interface_open", code="RESEARCH@abc0000011")
...
async def test_interface_open_rejects_an_unknown_code_type(call, opened):
    with pytest.raises(ToolError, match="RESEARCH@ / AREA@"):
        await call("interface_open", code="WIDGET@abc0000010")

    assert opened == []


async def test_interface_open_names_the_format_of_a_retired_code(call, opened):
    """Код старого формата — отказ, который учит, а не «страница не найдена»."""
    with pytest.raises(ToolError, match="retired 22-char format"):
        await call("interface_open", code="RESEARCH@abc0000000000000000012")
