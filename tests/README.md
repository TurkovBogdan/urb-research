...
**Heavy-тесты Alembic-миграций — только Postgres.** Миграции описаны в типах `postgresql.*` (JSONB/TIMESTAMP) и на SQLite не накатываются, поэтому по умолчанию такие тесты скипаются. Чтобы прогнать их на реальной базе, задайте DSN:
```dotenv
TEST_PG_DSN=postgresql://user:pass@host:port/dbname
```
